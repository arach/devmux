...
struct SettingsView: View {
                        ...
                        VStack(alignment: .leading, spacing: 20) {
                            ...
                            settingsRow("Scan root") {
                                VStack(alignment: .leading, spacing: 6) {
                                    ...
                                    Text("Scans for .devmux.json configs")
                                        .font(Typo.caption(10))
                                        .foregroundColor(Palette.textMuted)
                                }
                            }
    ...
    private var docsContent: some View {
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 0, pinnedViews: .sectionHeaders) {
                Section(header: stickyHeader("What is devmux?")) {
                ...
                Section(header: stickyHeader("Glossary")) {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        glossaryItem("tmux",
                            "Terminal multiplexer \u{2014} the engine behind devmux. It manages sessions, panes, and layouts. devmux configures it so you don\u{2019}t have to.")
                    }
                    .padding(.horizontal, 20)
                    .padding(.vertical, 12)
                }
                ...
                Section(header: stickyHeader("How it works")) {
                    VStack(alignment: .leading, spacing: 8) {
                        flowStep("1", "Create a .devmux.json in your project root")
                        flowStep("2", "devmux reads the config and builds a tmux session")
    ...
    private func resolveDocsFile(_ file: String) -> String {
        let devPath = "/Users/arach/dev/devmux/docs/\(file)"
